...
function BasicTextField({ input, placeholder }) {
    return <input {...input} placeholder={placeholder}></input>
}
    ...
    return (
        <select {...input}>
            {options.map(({ label, value }) => (
                <option key={value || '__NoKey'} value={value}>
                    {label}
                </option>
            ))}
        </select>
...
function DishForm(props) {
    ...
    return (
        <form onSubmit={handleSubmit} className="dishform">
            <p className="dishform__header">Submit-a-dish</p>
            <div className="dishform__fields-container">
                <div className="dishform__fields">
                    <Field
                        name="name"
                        label="Name"
                        placeholder="Chilli con Saturne"
                        component={WrappedBasicTextField}
                    ></Field>
                    <Field
                        name="preparation_time"
                        label="Preparation Time"
                        placeholder="01:45:00"
                        component={WrappedPreptimeField}
                    ></Field>
                    <Field
                        name="type"
                        label="Dish type"
                        options={typeOptions}
                        component={WrappedOptionsField}
                    ></Field>
                    {type === 'pizza' && (
                        <>
                            <Field
...
const validate = (data) => {
    ...
    if (!data.preparation_time) {
        ...
        if (/^\d{2}:\d{2}:\d{2}$/.test(data.preparation_time) === false) {
            errors.preparation_time =
                'Invalid preparation time - use the HH:MM:SS format, and fill out every field'
        }
    }
    if (!data.type) {
    ...
    if (data.type) {
        ...
        if (data.type === 'pizza') {
            ...
            } else if (false === /^\d+$/.test(data.no_of_slices)) {
                // weird test cos ! and then a regex is hard to read
            ...
            if (!data.slices_of_bread) {
                ...
                if (parseInt(data.slices_of_bread) < 1) {
                    errors.slices_of_bread =
                        'A sandwitch has to have at least one slice'
                }
            }
        }
    }
